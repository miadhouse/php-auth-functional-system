<?php
/**
 * Complete Subscription Management Functions
 * PHP 8.4 Pure Functional Script
 * Save as: includes/subscription.php
 */
 ...
 */
function format_subscription_status($status) {
    $status_map = [
        'active' => [
            'class' => 'success',
 ...
 */
function get_plan_type_color($plan_type) {
    $color_map = [
        'bronze' => 'warning',
        'silver' => 'secondary',
        'gold' => 'warning',
    ...
    return $color_map[$plan_type] ?? 'primary';
}
 ...
 *
 * @param array $subscription Subscription data
 * @return bool True if trial expires within 3 days
 */
function is_trial_expiring_soon($subscription) {
    if ($subscription['status'] !== 'trial' || !$subscription['trial_ends_at']) {
        return false;
    }
    
    $trial_end = strtotime($subscription['trial_ends_at']);
    $now = time();
    $days_left = ceil(($trial_end - $now) / 86400);
